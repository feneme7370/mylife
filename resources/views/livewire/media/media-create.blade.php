<div class="w-full">
    ...
    <x-pages.breadcrums.breadcrum title_1="Inicio" link_1="{{ route('dashboard') }}" title_2="Peliculas y Series"
        link_2="{{ route('media_dashboard') }}" title_3="Crear {{ $type_content[$type] }}"
    ...
    <form wire:submit.prevent="saveMedia"
        ...
        <div class="col-span-12 sm:col-span-6">
            <div class="flex">
                <x-pages.buttons.create-text
                    ...
                    wire:click="createActionModalActor" wire:loading.attr="disabled" />
        ...
        <div class="col-span-12 sm:col-span-6">
            <div class="flex">
                <x-pages.buttons.create-text
                    class="text-sm font-medium text-gray-700 hover:text-gray-500 hover:underline "
                    wire:click="createActionModalDirector" wire:loading.attr="disabled" />
                <x-pages.forms.label-form for="selected_media_directions" value="{{ __('Directores') }}" />
            </div>
        ...
        <div class="sm:col-span-3 col-span-6">
            <x-pages.forms.label-form for="status" value="{{ __('Estado de vision') }}" />
        ...
        <x-pages.modals.jetstream.dialog-modal wire:model="showCreateModalDirector">
            <x-slot name="title">
                {{ __('Formulario') }}
            </x-slot>
    
            <x-slot name="content">
                <form wire:submit="saveDirectorCreate"
                    class="sm:m-3 p-4 bg-white border border-purple-200 rounded-lg shadow-sm sm:p-8  ">
    
                    <div>
                        <x-pages.forms.label-form for="name_director" value="{{ __('Nombre') }}" />
                        <x-pages.forms.input-form id="name_director" type="text" placeholder="{{ __('Nombre') }}"
                            wire:model="name_director" />
                        <x-pages.forms.input-error for="name_director" />
                    </div>
            ...
            <x-slot name="footer">
                <x-pages.buttons.primary-btn title="Guardar" wire:click="saveDirectorCreate"></x-pages.buttons.primary-btn>
            </x-slot>
        </x-pages.modals.jetstream.dialog-modal>
        {{-- end modal action create collection --}}

        {{-- modal action create collection --}}
        <x-pages.modals.jetstream.dialog-modal wire:model="showCreateModalActor">
            <x-slot name="title">
                {{ __('Formulario') }}
            </x-slot>

            <x-slot name="content">
                <form wire:submit="saveActorCreate"
                    class="sm:m-3 p-4 bg-white border border-purple-200 rounded-lg shadow-sm sm:p-8  ">

                    <div>
                        <x-pages.forms.label-form for="name_actor" value="{{ __('Nombre') }}" />
                        <x-pages.forms.input-form id="name_actor" type="text" placeholder="{{ __('Nombre') }}"
                            wire:model="name_actor" />
                        <x-pages.forms.input-error for="name_actor" />
                    </div>
            ...
            <x-slot name="footer">
                <x-pages.buttons.primary-btn title="Guardar" wire:click="saveActorCreate"></x-pages.buttons.primary-btn>
            </x-slot>
        </x-pages.modals.jetstream.dialog-modal>
        {{-- end modal action create collection --}}
    ...
    <script type="text/javascript">
        document.addEventListener('livewire:initialized', function () {
            ...
            function loadJavascript(){
                $('#selected_media_actors').select2().on("change", function(){
                    @this.set("selected_media_actors", $(this).val());
                });
                $('#selected_media_directors').select2().on("change", function(){
                    @this.set("selected_media_directors", $(this).val());
                });
                $('#selected_media_collections').select2().on("change", function(){
                    @this.set("selected_media_collections", $(this).val());
                });
            }
            loadJavascript();
