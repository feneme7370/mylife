<div class="w-full">
    ...
    link_2="{{ route('media_dashboard') }}" title_3="Actores" link_3="{{ route('media_actor_list') }}" />

<x-pages.menus.title-and-btn>

    @slot('title')
    <x-pages.titles.title-pages title="Actores" />
    @endslot
            ...
            <div class="flex items-center justify-between gap-2 overflow-x-auto">
                    ...
                    <div class="">
                        <p class="text-sm font-medium text-gray-900 truncate ">
                            <a class="hover:underline" href="{{ route('media_library', ['a' => $item->uuid]) }}">{{ $item->name }}</a>
                        </p>
                    ...
                    <a href="{{ route('media_library', ['a' => $item->uuid]) }}" class="bg-purple-900 text-purple-50 text-xs font-medium me-2 px-2.5 py-0.5 rounded-lg">
                        {{ $item->medias->count() }}
                    </a>
...
{{-- Paginacion --}}
<div class="mt-2">{{ $actors->onEachSide(1)->links() }}</div>
{{-- end Paginacion --}}
</div>
...
<x-slot name="content">
    <form wire:submit="saveActorCreate" class="sm:m-3 p-4 bg-white border border-purple-200 rounded-lg shadow-sm sm:p-8  ">
        ...
        <div>
          ...
          <div class="col-span-12">
            <x-pages.forms.label-form for="cover_image_url" value="{{ __('URL del actor') }}" />
            <x-pages.forms.input-form id="cover_image_url" type="text" placeholder="{{ __('URL del actor') }}" wire:model="cover_image_url"
                  />
...
<x-slot name="content">
    <form wire:submit="saveActorEdit" class="sm:m-3 p-4 bg-white border border-purple-200 rounded-lg shadow-sm sm:p-8  ">
   <div>
    <x-pages.forms.label-form for="name" value="{{ __('Nombre') }}" />
    <x-pages.forms.input-form id="name" type="text" placeholder="{{ __('Nombre') }}" wire:model="name"
          />
...
<x-slot name="content">
    <form wire:submit="deleteActor" class="sm:m-3 p-4 bg-purple-50 border border-purple-200 rounded-lg shadow-sm sm:p-8  text-center">
        
        <p class="mb-5">Desea borrar el actor?</p>
