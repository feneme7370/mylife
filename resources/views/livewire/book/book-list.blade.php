<div class="w-full ">
    ...
    link_2="{{ route('book_dashboard') }}" title_3="Lista de {{ App\Models\Book::title() }}" link_3="{{ route('book_list') }}" />
            ...
            <section class="w-full p-1 border border-purple-300 bg-gray-50 rounded-md mt-1 flex flex-col justify-between items-start">
                ...
                <div class="grid grid-cols-12 gap-3 w-full">
                    ...
                    <div class="w-full col-span-8">
                        <div class="text-sm md:text-base font-semibold hover:underline line-clamp-3"><a href="{{ route('book_view', ['uuid' => $item->uuid]) }}">{{ $item->title }}</a></div>
                ...
                <div class="flex flex-col gap-1 mt-1">
                    <div class="flex gap-1 flex-wrap">
                        @foreach ($item->book_collections as $collection_item)
                            <a href="{{ route('book_library', ['c' => $collection_item->uuid]) }}" class="hover:underline text-xs md:text-sm text-gray-500 italic bg-gray-200 rounded-md px-1 border border-purple-200">{{ $collection_item->name }}</a>
                        @endforeach
                    </div>
                    <div class="flex gap-1 flex-wrap">
                ...
                <div class="w-full">
                    ...
                    <div class="flex justify-between">
                            ...
                            <div class="flex items-center justify-center gap-1">
                                <a href="{{ route('book_edit', ['type' => $item->book_type, 'uuid' => $item->uuid]) }}" class="font-medium text-gray-600  hover:underline"><x-pages.buttons.edit-text/></a>
    ...
    {{-- <table class="w-full overflow-hidden text-sm text-left rtl:text-right text-gray-500 ">
        ...
        <tbody>
            ...
            <tr class="bg-white border-b border-purple-200 hover:bg-purple-50">
                ...
                <td class="px-6 py-4">
                    <div class="flex flex-col gap-1">
                        <span>{{ $item->book_type == null ? '' : $type_content[$item->book_type] }} ({{ \Carbon\Carbon::parse($item->release_date)->year }})</span>
