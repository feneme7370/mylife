<div class="w-full">
    ...
    link_2="{{ route('recipe_dashboard') }}" title_3="Categorias" link_3="{{ route('recipe_category_list') }}" />
            ...
            <div class="flex items-center justify-between gap-2">
                    ...
                    <div class="">
                        <p class="text-sm font-medium text-gray-900 truncate ">
                            <a class="hover:underline" href="{{ route('recipe_list', ['c' => $item->uuid]) }}">{{ $item->name }}</a>
                        </p>
                    </div>

                    <div class="hidden sm:flex gap-1">
                        {{-- @foreach ($item->recipes as $item_recipe)
                        <a href="{{ route('recipe_library', ['q' => $item_recipe->title]) }}" class="bg-purple-900 text-purple-50 text-xs font-medium me-2 px-2.5 py-0.5 rounded-lg">
                            {{ $item_recipe->title }}
                        </a>
                        @endforeach --}}
                    </div>
...
<x-slot name="content">
    <form wire:submit="saveCategoryCreate" class="sm:m-3 p-4 bg-white border border-purple-200 rounded-lg shadow-sm sm:p-8  ">
        ...
        <div>
          ...
          <div class="col-span-12">
            <x-pages.forms.label-form for="cover_image_url" value="{{ __('URL de la categoria') }}" />
            <x-pages.forms.input-form id="cover_image_url" type="text" placeholder="{{ __('URL de la categoria') }}" wire:model="cover_image_url"
                  />
...
<x-slot name="content">
    <form wire:submit="saveCategoryEdit" class="sm:m-3 p-4 bg-white border border-purple-200 rounded-lg shadow-sm sm:p-8  ">
   <div>
    <x-pages.forms.label-form for="name" value="{{ __('Nombre') }}" />
    <x-pages.forms.input-form id="name" type="text" placeholder="{{ __('Nombre') }}" wire:model="name"
          />
...
<x-slot name="content">
    <form wire:submit="deleteCategory" class="sm:m-3 p-4 bg-purple-50 border border-purple-200 rounded-lg shadow-sm sm:p-8  text-center">
        
        <p class="mb-5">Desea borrar la categoria?</p>
